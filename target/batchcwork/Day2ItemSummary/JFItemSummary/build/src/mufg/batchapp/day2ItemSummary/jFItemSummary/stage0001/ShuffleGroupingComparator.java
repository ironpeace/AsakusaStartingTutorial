...
@SuppressWarnings("rawtypes") public class ShuffleGroupingComparator implements RawComparator<ShuffleKey> {
    @Override public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2, int l2) {
        ...
        switch(segmentId1) {
            case 1:
            case 2:
                size1 = com.asakusafw.runtime.value.LongOption.getBytesLength(b1, s1 + o1, l1 - o1);
                size2 = com.asakusafw.runtime.value.LongOption.getBytesLength(b2, s2 + o2, l2 - o2);
                diff = com.asakusafw.runtime.value.LongOption.compareBytes(b1, s1 + o1, size1, b2, s2 + o2, size2);
                if(diff != 0) return diff;
                o1 += size1;
                o2 += size2;
                break;
